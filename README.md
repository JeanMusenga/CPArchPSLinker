...
- **`2,534_Architectural_Solutions.xlsx`** – Contains **2,534 randomly selected architectural solutions** from the set of **10,423 architectural solutions (ARPs)** curated in our previous study (de Dieu et al., 2025b). These solutions were paired with GitHub architectural problems exctracted from GitHub commits and issues to construct the **irrelevant (negative) ⟨architectural problem, solution⟩ pairs** included in the benchmark dataset.
- **`arch_terms.json`** – Contains a domain-specific architectural knowledge base used for feature engineering and rule-based association scoring. It organizes problem-related and solution-related architectural terminology into structured categories (e.g., performance, scalability, security, architectural patterns, tactics, frameworks, APIs, protocols, refactoring strategies, and recovery mechanisms). This file supports the computation of the architectural association score (A) by enabling rule-based detection of conceptual overlap between GitHub-derived architectural problems and candidate solutions extracted from Stack Overflow. 
